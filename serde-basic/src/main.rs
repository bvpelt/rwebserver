...
fn main() {
    ...
    println!("config to yaml serialized: {}", serialized);

    let deserialized: ServerConfig = serde_yaml::from_str(&serialized).unwrap();
    println!("yaml to config to deserialized: {:?}", deserialized);

    println!("JSON");
    println!("To and from JSON");
    let serialized = serde_json::to_string(&config).unwrap();
    println!("config to json serialized: {}", serialized);

    let deserialized: ServerConfig = serde_json::from_str(&serialized).unwrap();
    println!("json to config to deserialized: {:?}", deserialized);
}
